...
export default async function PoliticsPage() {
  const { politics } = await fetchHomePagePosts()
  const posts = politics.map(mapWpPost)
  ...
  return (
    <section className="pt-44   w-full">
      <div className="w-full max-w-[1920px] mx-auto px-mobile-safe">
        ...
        <div className="flex flex-col md:flex-wrap md:flex-row 2xl:grid grid-cols-5 justify-start gap-6 md:gap-8 lg:gap-10  mb-10 ">
          {posts.length > 0 ? (
            posts.map((post) => {
