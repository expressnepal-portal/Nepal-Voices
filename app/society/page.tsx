...
export default async function SocietyPage() {
  const { society } = await fetchHomePagePosts();
  const posts = society.map(mapWpPost);
      ...
      <div className="w-full max-w-[1920px] mx-auto px-mobile-safe">
        ...
        <div className="flex flex-col md:flex-wrap md:flex-row 2xl:grid grid-cols-5 justify-start gap-6 md:gap-8 lg:gap-10">
          {posts.length > 0 ? (
            ...
            <div className=" w-full text-2xl md:text-3xl text-center text-gray-600 flex items-center justify-center h-[50vh]">
